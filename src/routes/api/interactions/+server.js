...
async function fetchAndSendMatchData(accountId, token, applicationId) {
    ...
    try {
        ...
        const latestMatch = Array.isArray(recentMatches) && recentMatches.length > 0 ? recentMatches[0] : null;
        ...
        try {
            ...
            console.error("Webhook fallback delivery failed:", webhookError.response?.data || webhookError.message);
        }
    }
}
